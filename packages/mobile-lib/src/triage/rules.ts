...
export const evaluateTriage = (answers: TriageAnswer[]): TriageResult => {
  ...
  const urgentQuestions = positiveQuestions.filter(question => question.urgentOnPositive);
  const redFlags = blockingQuestions.map(question => question.category);

  const canContinueSelfTest = redFlags.length === 0 && unansweredQuestionIds.length === 0;
  ...
  return {
    ...
    warnings: [
      ...redFlags.map(category => ({
        ...
        message: 'A safety triage answer indicates professional evaluation is recommended.',
        severity: urgentQuestions.some(question => question.category === category)
          ? 'critical'
          : 'warning',
        source: 'triage',
      }) as const),
